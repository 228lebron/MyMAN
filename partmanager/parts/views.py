...
class PartDetailView(LoginRequiredMixin, UserContextMixin, UserPassesTestMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        last_price = quotas.order_by('date').last().price if quotas.exists() else None

        prices, dates, suppliers, lead_times, quantity = get_price_data(part)
        prices_d, dates_d, lead_times_d, quantity_d = get_price_data_d(part)
        chart = get_plot(dates_d, prices_d, 'График цен', 'Дата', 'Цена USD')
